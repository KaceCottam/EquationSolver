...
protected:
	...
	//returns a pair of pairs, in the format {{left,right},{leftMostIndex,rightMostIndex}}
	virtual std::pair<std::pair<T, T>, std::pair<int, int>> DiscoverInput(int index, std::string& in) = 0;
		...
		case End:
		{
			...
			for (auto& i : UnaryOperators)
			{
				...
				if (discoverIndex == std::string::npos)
				{
					...
					discovery.first = DiscoverInput(discoverIndex, inputString);
				}
			}
...
public:
	...
	{
		return DoParse(inputString, output, steps); //Add check functions
	}
};